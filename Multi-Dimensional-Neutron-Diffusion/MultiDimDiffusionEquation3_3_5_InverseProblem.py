...
def phi_analytical(x):
    return 1 / 2 * np.cos(x[:, 0:1] * np.pi / a) * np.exp((k_n - 1) * x[:, 1:2] / l_n)
...
# 定义数据
data = dde.data.TimePDE(geomtime, pde, [bc, ic, observe_y], num_domain=300, num_boundary=200, num_initial=100,
                        solution=None,
...
# 定义求解器
model.compile("adam", lr=0.001, loss_weights=[1, Pb, Pi, Pc], external_trainable_variables=[k_eff])
